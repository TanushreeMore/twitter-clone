...
function Tweet(props) {
  ...
  // Function to fetch comments and update state
  async function populateComments() {
    ...
    if (data.status === "ok") {
      if (Array.isArray(data.tweet.comments) && data.tweet.comments.length > 0) {
  ...
  // Effect hook to fetch comments on component mount and when loading state changes
  useEffect(() => {
    populateComments();
  }, [loading]);
  ...
  // Event handler for retweet button click
  ...
  // Functions to handle state changes for liking and retweeting
  const handleSetLike = () => {
  ...
  // Event handler for submitting like/unlike
  ...
  // Event handler for deleting a tweet
  const deleteTweet = (e) => {
    e.preventDefault();
  ...
  // Event handler for editing a tweet
  const editTweet = (e) => {
    e.preventDefault();
